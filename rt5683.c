...
#define FixedType
/**
* Below gpio numbers are based on realtek's dev platform, and should be
* modified according to dev platform respectivelly or parsing from DTS.
*/
#define BTN_IRQ_GPIO 165
#define JACK_IRQ_GPIO 166
...
struct rt5683_priv {
	...
	struct delayed_work btn_detect_work;
	int sysclk;
	int sysclk_src;
	int lrck;
	int bclk;
...
static irqreturn_t rt5683_btn_irq_handler(int irq, void *dev_id)
{
	struct rt5683_priv *rt5683  = dev_id;
	
	pr_info("%s:BTN IRQ detected!",__func__);
	schedule_work(&rt5683->btn_detect_work);

	return IRQ_HANDLED;
}

static irqreturn_t rt5683_jack_type_irq_handler(int irq, void *dev_id)
{
	struct rt5683_priv *rt5683  = dev_id;
	
	pr_info("%s:JD IRQ detected!",__func__);
	schedule_work(&rt5683->hs_detect_work);

	return IRQ_HANDLED;
}

int rt5683_set_jack_detect(struct snd_soc_component *component,
	struct snd_soc_jack *hs_jack, struct snd_soc_jack *btn_jack)
{
	struct rt5683_priv *rt5683 = snd_soc_component_get_drvdata(component);

	rt5683->hs_jack = hs_jack;
	rt5683->btn_jack = btn_jack;
	
	rt5683_jack_type_irq_handler(0, rt5683);
...
int rt5683_button_detect(struct snd_soc_component *component)
{
	...
	if ((val_00B6 & 0x10) == 0x00){
		...
		else //Abnormal Bottom push
		{
			...
			pr_info("[DBG] Abnormal Bottom push, 0x00b6:0x%x\n",val_00B6);
		}
	} else {
...
void rt5683_check_btn_interrupt_event(struct work_struct *work)
{
	struct rt5683_priv *rt5683 =
		container_of(work, struct rt5683_priv, btn_detect_work.work);
	...
	val_070c &= 0x77;
	val_070d &= 0x77;
	pr_info("val_00be:0x%x,val_070c:0x%x,val_070d:0x%x\n",val_00be,val_070c,val_070d);
	/* Status of InLine Command Trigger */
	if (((val_00be & 0x80)==0x80)){
	...
	snd_soc_jack_report(rt5683->btn_jack, report, SND_JACK_BTN_0 |
...
int rt5683_headset_detect(struct snd_soc_component *component)
{
	...
	while(i < 5){
		...
		if (val_2b03 == 0x1 || val_2b03 == 0x2 || val_2b03 == 0x3)
			break; 
		if (i == 5 && val_2b03 == 0x0){
			pr_info("detect jack type again\n");
			schedule_work(&rt5683->hs_detect_work);
		}
	}
...
void rt5683_jack_type_interrupt_event(struct work_struct *work)
{
	struct rt5683_priv *rt5683 =
		container_of(work, struct rt5683_priv, hs_detect_work.work);
	unsigned int val_00bd, val_2b03, val;
	int report=0,i;
	
	for(i=0;i<5;i++){
		msleep(1);
		regmap_read(rt5683->regmap, 0x00BD, &val_00bd);
		pr_info("val_00bd:0x%x\n",val_00bd);
	}

	/* JD Status Confirm */
	if (((val_00bd & 0x30)==0x0))
		report = rt5683_headset_detect(rt5683->component);
	else
		report = 0;
		    ...
		    const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&rt5683->btn_detect_work, rt5683_check_btn_interrupt_event);
	INIT_DELAYED_WORK(&rt5683->hs_detect_work, rt5683_jack_type_interrupt_event);
	
	irq_num = gpio_to_irq(BTN_IRQ_GPIO);
	ret = request_irq(irq_num, rt5683_btn_irq_handler,
		IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING | IRQF_ONESHOT,
			"5683", rt5683);
	if (ret < 0)
		dev_err(&i2c->dev, "BTN IRQ request failed!\n");

	irq_num = gpio_to_irq(JACK_IRQ_GPIO);
	ret = request_irq(irq_num, rt5683_jack_type_irq_handler,
		IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING | IRQF_ONESHOT,
			"5683", rt5683);
	if (ret < 0)
